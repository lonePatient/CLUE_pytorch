    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        if pred_task == 'copa':
            ...
            while (i < len(pred_logits) - 1):
                ...
                i += 2
        output_submit_file = os.path.join(pred_output_dir, prefix, f"{args.task_name}_prediction.json")
        output_logits_file = os.path.join(pred_output_dir, prefix, f"{args.task_name}_logits")
        # 保存标签结果
...
def main():
    ...
    parser.add_argument("--do_predict", action='store_true',help="Whether to run inference on the test set.")
    parser.add_argument(
